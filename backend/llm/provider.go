...
// Analyzer is the interface the handler depends on. It enables test doubles and
// provider fallback (see Chain).
...
// analyzeChunks splits text into chunks, runs analyzeChunk concurrently over
// each, and merges the results. Chunking and merging are shared across
// providers; only the per-chunk call (analyzeChunk) is provider-specific.
func analyzeChunks(text string, analyzeChunk func(string) (AnalysisResult, error)) (AnalysisResult, error) {
	...
	for i, chunk := range chunks {
		...
		go func(i int, chunk string) {
			defer wg.Done()
			results[i], errs[i] = analyzeChunk(chunk)
		}(i, chunk)
	}
	wg.Wait()
...
// parseAnalysisJSON unmarshals a model's raw text response into an
// AnalysisResult, tolerating markdown code fences some models add despite
// instructions.
func parseAnalysisJSON(raw string) (AnalysisResult, error) {
	...
	if err := json.Unmarshal([]byte(raw), &result); err != nil {
		return AnalysisResult{}, fmt.Errorf("parsing analysis JSON: %w (raw: %s)", err, raw)
	}
	return result, nil
}
